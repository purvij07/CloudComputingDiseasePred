...
def predict_disease():
    ...
    for i in range(len(symptom_names)):
        symptom = 1 if request.form.get(f'{i}') else 0
        symptoms.append(symptom)

    # prediction
    result = model.predict(np.array(symptoms).reshape(1, -1))

    return render_template('index.html', result=result[0], symptom_names=symptom_names)
